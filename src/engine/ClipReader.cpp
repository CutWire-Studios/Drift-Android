...
#ifdef Q_OS_ANDROID
    // The path below is VAAPI, which is Linux-only and configured out of the Android FFmpeg.
    // MediaCodec takes its place, but deliberately not as a hwaccel — see
    // tryOpenMediaCodecDecoder. Anything it declines leaves m_hwAccelDisabled set, which is
    // already how this reader says "software from here on".
