                        ...
                        AVRational frameRate, QString *errorOut)
{
    ...
    if (!m_fmt)
        return fail(QStringLiteral("Could not create the reversed container"));
    ...
    if (!m_stream)
        return fail(QStringLiteral("Could not create the reversed stream"));

    m_ctx = avcodec_alloc_context3(codec);
    if (!m_ctx)
        return fail(QStringLiteral("Could not allocate the reversed encoder"));
    ...
    if (!(m_fmt->oformat->flags & AVFMT_NOFILE)) {
        if (avio_open(&m_fmt->pb, tmpUtf8.constData(), AVIO_FLAG_WRITE) < 0)
            return fail(QStringLiteral("Could not open the reversed file for writing"));
    }
    if (avformat_write_header(m_fmt, nullptr) < 0)
        return fail(QStringLiteral("Could not write the reversed header"));

    m_pkt = av_packet_alloc();
    m_frame = av_frame_alloc();
    if (!m_pkt || !m_frame)
        return fail(QStringLiteral("Could not allocate reversed frame buffers"));

    m_frame->format = AV_PIX_FMT_YUV420P;
    m_frame->width = m_ctx->width;
    m_frame->height = m_ctx->height;
    if (av_frame_get_buffer(m_frame, 0) < 0)
        return fail(QStringLiteral("Could not allocate the reversed frame"));
...
bool ProxyEncoder::writeFrame(const AVFrame *src, int64_t pts, QString *errorOut)
{
    if (!m_ctx || !m_frame) {
        if (errorOut)
            *errorOut = QStringLiteral("Reversed writer is not open");
        return false;
    }
    ...
    if (!m_sws) {
        if (errorOut)
            *errorOut = QStringLiteral("Could not convert a frame for the reversed encoder");
        return false;
    }
    if (av_frame_make_writable(m_frame) < 0) {
        if (errorOut)
            *errorOut = QStringLiteral("Could not make the reversed frame writable");
        return false;
    }
    ...
    if (avcodec_send_frame(m_ctx, m_frame) < 0) {
        if (errorOut)
            *errorOut = QStringLiteral("Reversed encoder rejected a frame");
        return false;
    }
...
bool ProxyEncoder::drainPackets(QString *errorOut)
{
    for (;;) {
        ...
        if (rc < 0) {
            if (errorOut)
                *errorOut = QStringLiteral("Failed to read an encoded reversed packet");
            return false;
        }
        ...
        if (wrc < 0) {
            if (errorOut)
                *errorOut = QStringLiteral("Failed to write a reversed packet");
            return false;
        }
    }
}
...
bool ProxyEncoder::finish(QString *errorOut)
{
    ...
    if (avcodec_send_frame(m_ctx, nullptr) < 0) {
        if (errorOut)
            *errorOut = QStringLiteral("Could not flush the reversed encoder");
        return false;
    }
    ...
    if (av_write_trailer(m_fmt) < 0) {
        if (errorOut)
            *errorOut = QStringLiteral("Could not write the reversed trailer");
        return false;
    }
    ...
    if (!QFile::rename(m_tmpPath, m_path)) {
        if (errorOut)
            *errorOut = QStringLiteral("Could not move the reversed clip into place");
        return false;
    }
                    ...
                    const std::function<bool(double)> &onProgress)
{
    ...
    if (avcodec_open2(source.dec, codec, nullptr) < 0)
        return fail(QStringLiteral("Could not open the decoder"));
    if (source.dec->width <= 0 || source.dec->height <= 0)
        return fail(QStringLiteral("The clip has no usable video size"));
    ...
    if (!frame || !packet) {
        ...
        encoder.abort();
        return fail(QStringLiteral("Could not allocate decode buffers"));
    }
    ...
    while (upperTs >= coverInTs) {
        ...
        if (onProgress) {
            ...
            if (!onProgress(std::clamp(done, 0.0, 1.0))) {
                cleanup();
                encoder.abort();
                return fail(QStringLiteral("Reversing cancelled"));
            }
        }
    }
    ...
    if (!wroteAny) {
        encoder.abort();
        return fail(QStringLiteral("No frames could be decoded from this clip"));
    }
