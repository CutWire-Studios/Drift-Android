...
//
// Every value below is fixed by the OpenGL ES 3.0 / OpenGL 3.x specification and identical on both,
// so this is a declaration gap rather than a portability compromise. The context is checked to be
// ES 3.0 at creation (initGlObjects), which is what actually guarantees they are usable.
#ifndef GL_RED
...
// Every shader in the project — the builtins below, the compositor's, and all 102 shipped effect
// and transition packages — is written as `#version 330 core`. Android has no desktop GL, so the
// version line is swapped for `#version 300 es` and the default precision qualifiers ES requires
// (and desktop GL does not accept) are prepended.
//
// The bodies need nothing else: none of them use gl_FragColor, texture2D, derivatives, textureLod,
// texelFetch or a non-constant loop bound, and layout(location=) on a vertex input is core in ES
// 3.00. So this stays a version-line rewrite rather than a translation.
//
// Done here, at the three compile sites, rather than by rewriting the .frag files: the package
// shaders are shared with the desktop build, and the ~7 inline shader strings scattered across
// GpuCompositor and GpuEffectExecutor would be missed by any file-level pass.
...
// Two explicit overloads rather than one taking QByteArray: a `const char *` literal converts to
// both QByteArray and QString, so a single parameter type would make every call site ambiguous.
...
bool GlRuntime::initGlObjects()
{
    ...
    // The default format is set in main() so this context and the Qt Quick scene graph's agree on
    // the version, which is a precondition for sharing textures between them.
