        ...
        delegate: Item {
            ...
            Image {
                ...
                onStatusChanged: {
                    if (tile.index !== 0)
                        return
                    // Error clears it too: `loading` is !firstFrameReady, so leaving it
                    // false on a failed decode pinned the skeleton over the clip forever
                    // rather than falling through to the placeholder chrome.
